...
CC_NAME="rating"
CC_SRC_PATH="../contract/rating"
CC_RUNTIME_LANGUAGE="golang"
CC_VERSION="1"
CHANNEL_NAME="ratingchannel"
...
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"Args":["InitLedger","User1","Cleaner"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt
sleep 3

## TEST2 : Check if Ledger exists
infoln "TEST2 : Check if Ledger exists"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["LedgerExists","User1:Cleaner"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt

## TEST3 : Read Ledger
infoln "TEST3 : Read Ledger"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["ReadLedger","User1:Cleaner"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt

## TEST4 : GetAllRatings
infoln "TEST4 : GetAllRatings"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["GetAllRatings","User1:Cleaner"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt

## TEST5 : RegisterRate
infoln "TEST5 : RegisterRate"
set -x
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n ${CC_NAME} $PEER_CONN_PARMS -c '{"Args":["RegisterRate","User1:Cleaner","4","User3"]}' >&log.txt

{ set +x; } 2>/dev/null
cat log.txt

## TEST6 : Read Ledger
infoln "TEST6 : Read Ledger in which rate is registered"
set -x
peer chaincode query -C $CHANNEL_NAME -n ${CC_NAME} -c '{"Args":["ReadLedger","User1:Cleaner"]}' >&log.txt
{ set +x; } 2>/dev/null
cat log.txt